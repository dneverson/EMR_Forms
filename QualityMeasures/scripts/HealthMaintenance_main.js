...
function fnHMGetImmunizations() {
	// Variables
	var rawImmunList = EvaluateMel('{IMMUN_GETLIST("", "Last")}'); // Latest immunizations	
	var immunHTML = "";
	var tdHTML = "";
...
function fnHMGetScreenings() {
	...
	var observations = [
	  ...
	  ["Cologuard", "COLOGUARDRES"],
	  ["Dental Exam", "DMDENTEXLAST"],
	  ["Diabetic Eye Exam", "DIAB EYE EX"],
	...
	var tobaccoObs = EvaluateMel('{fnUtilityGetMostRecentObsName(ARRAY("SMOK STATUS","TOBACCO USE","SMOKES","ORALTOBACUSE"))}');
	if (tobaccoObs != "") {
	...
	// && EvaluateMel('{OBSPREV("MAMMO_EXEMPT")}') == ""
	if (EvaluateMel("{Patient.Sex}") == "Female") {
...
/*function fnHMGetLabTests() {
	...
	var ekgObs = EvaluateMel('{fnUtilityGetMostRecentObsName(ARRAY("EKG","EKG INTERP"))}');
	...
	if (EvaluateMel("{Patient.Sex}") == "Female" && EvaluateMel('{OBSPREV("PAP_EXEMPT")}') == "") {
...
function fnHMGetDiseaseMgmt() {
	...
	// Check for diabetes
	var testing = EvaluateMel('{checkDxList("E11")}');
	debugBox.innerHTML = testing;
	if (EvaluateMel('{checkDxList("E10")}') != 'FALSE' || EvaluateMel('{checkDxList("E11")}') != 'FALSE') {
...
function fnGetObsData(obsName, obsTerm) {
	var obsData = [];
	var obsValue = EvaluateMel('{LASTOBSVALUE("' + obsTerm + '")}');
	var obsDate = EvaluateMel('{LASTOBSDATE("' + obsTerm + '")}');
...
function fnRequestColonoscopy(btnObject) {
	...
	// Add to chart note
	EvaluateMel('{DOCUMENT.MeasuresChartNote = DOCUMENT.MeasuresChartNote + "   - Request sent to Referral Specialist to schedule a colonoscopy screening appointment" + HRET}');
}
