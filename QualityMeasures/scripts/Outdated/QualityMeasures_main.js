...
function fnTobaccoScreening(btnElement, btnText) {
	...
	var modalBody = "<h3>Please indicate the patient's tobacco use:</h3>" +
									...
									"	<div class='radio'><label><input type='radio' name='tobaccoScreen' value='Current'" + currentUse + "> Current</label></div>" +
									"	<div class='radio'><label><input type='radio' name='tobaccoScreen' value='Former'" + formerUse + "> Former</label></div>" +
									"	<div class='radio'><label><input type='radio' name='tobaccoScreen' value='Never'" + neverUse + "> Never</label></div>" +
...
function fnUpdateTobaccoUse(btnElement, btnText) {
	...
	if ( tobaccoUse == "" ) {
		Mel.eval('{userok("Please select a use level"');
	}
	else {
		Mel.eval('{OBSNOW("TOBACCO USE", "' + tobaccoUse + '")}');
		fnSetSuccessTd(btnElement, btnText, "Tobacco Use Updated");
		fnCloseModal();
	}
}
