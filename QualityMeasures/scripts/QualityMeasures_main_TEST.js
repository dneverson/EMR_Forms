...
function fnGetPtInfo() {
	...
	var ptPCP = Mel.eval("{Patient.PCP}");
...
// *** TOBACCO SCREENING SECTION *** \\
	...
	function fnTobaccoScreening(btnElement, btnText) {
		...
		var formerUse = "";
		var neverUse = "";
		var tobaccoObsTerm = Mel.eval('{fnUtilityGetMostRecentObsName(ARRAY("TOBACCO USE","SMOKES","ORALTOBACUSE","SMOK STATUS"))}');
		var tobaccoUsePrev = Mel.eval('{LASTOBSVALUEDATE("' + tobaccoObsTerm + '")}');
		var tobaccoUse = Mel.eval('{OBSPREV("' + tobaccoObsTerm + '")}');
		if (tobaccoUse == "") {
...
function fnAddPHQ9(btnElement, btnText) {
	Mel.eval('{ADD_FORM_COMP("Enterprise\\VMC\\Behavioral Health", "PHQ-9 Depression", "AFTER_CURRENT", "OPEN", "")}');
	fnSetSuccessTd(btnElement, btnText, "false");
}

// Order Urine Protein screening
function fnOrderUrineProtein(btnElement, btnText) {
	Mel.eval('{MEL_ADD_ORDER("T", "Laboratory", "Microalbumin", "", "ICD10-Z00.00", "Routine gen medical exam at health care facility", "Urine", "", "N", USER.LOGINNAME, "")}');
...
function fnOrderDBEyeExam(btnElement, btnText) {
  ...
  Mel.eval('{DOCUMENT.DIAB_EYE_EX_NOTE = "A diabetic retinal eye exam is recommended for this patient and has been ordered."}');
	fnSetSuccessTd(btnElement, btnText, "Diabetic retinal eye exam was recommended and has ben ordered for this patient.");
}
